...
menu.callback = (i:Input, e:Event) => {
    let num:number =  parseInt(i.input.value, 10);

    let hex:number = parseInt(i.input.value, 16);
    switch(i.id) {
        ...
        case "Déplacer": vue.movable = i.checked; break;
        case "Fermer": vue.closable = i.checked; break;
        case "Tailler": vue.resizable = i.checked; break;
        case "Couleur fond": vue.background.color = hex ;break;
        case "Couleur bord": vue.border.color = hex; break;
        default:
...
let watcher = new Proxy(vue, modifVue);
